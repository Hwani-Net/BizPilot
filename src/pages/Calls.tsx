...
const stateConfig: Record<AgentState, { label: string; color: string; ringColor: string; bgColor: string }> = {
  idle: { label: "대기중", color: "bg-[hsl(var(--text-muted))]", ringColor: "bg-[hsl(var(--text-muted))/0.3]", bgColor: "bg-[hsl(var(--bg-elevated))]" },
  listening: { label: "수신중", color: "bg-[hsl(var(--accent))]", ringColor: "bg-[hsl(var(--accent))/0.3]", bgColor: "bg-[hsl(var(--accent))/0.1]" },
  speaking: { label: "응답중", color: "bg-[hsl(var(--primary))]", ringColor: "bg-[hsl(var(--primary))/0.3]", bgColor: "bg-[hsl(var(--primary))/0.1]" },
  processing: { label: "처리중", color: "bg-rose-500", ringColor: "bg-rose-500/30", bgColor: "bg-rose-500/10" },
};
  ...
  return (
    <div className="v0-glass rounded-xl p-6">
      <div className="flex flex-col sm:flex-row items-center gap-6">
        <button onClick={cycleState} className="relative cursor-pointer bg-transparent border-0 p-0 outline-none">
          <div className={cn("relative w-28 h-28 rounded-full flex items-center justify-center transition-colors duration-500", config.bgColor)}>
            {isActive && (
              <>
                <div className={cn("absolute inset-0 rounded-full pulse-ring", config.ringColor)} />
                <div className={cn("absolute inset-2 rounded-full pulse-ring", config.ringColor)} style={{ animationDelay: "0.5s" }} />
              </>
            )}
            <div className={cn("w-16 h-16 rounded-full flex items-center justify-center relative z-10 transition-colors duration-300", config.color)}>
              {isActive ? (
        ...
        <div className="flex flex-col items-center sm:items-start gap-3 flex-1">
          ...
          <p className="text-sm text-[hsl(var(--text-muted))] text-center sm:text-left">
            {isActive
              ...
              : "에이전트가 비활성 상태입니다. 시작 버튼을 눌러 전화 수신을 시작하세요."
            }
          </p>
        ...
        {isActive && (
          <div className="hidden sm:flex flex-col gap-2 text-right">
            <div className="text-xs text-[hsl(var(--text-muted))]">통화 시간</div>
            <div className="text-2xl font-mono font-bold text-[hsl(var(--text))] number-display">03:45</div>
            <div className="text-xs text-[hsl(var(--text-muted))]">오늘 처리: 23건</div>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="v0-glass rounded-xl p-5 flex flex-col h-[500px]">
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div>
          <h3 className="text-sm font-semibold text-[hsl(var(--text))]">실시간 대화</h3>
          <p className="text-xs text-[hsl(var(--text-muted))] mt-0.5">AI 에이전트와 고객 간의 통화 내용</p>
        </div>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-[hsl(var(--accent))/0.15]">
          <div className="w-1.5 h-1.5 rounded-full bg-[hsl(var(--accent))] animate-pulse" />
          <span className="text-xs font-medium text-[hsl(var(--accent))]">실시간</span>
        </div>
      </div>
          ...
          {transcriptMessages.map((msg) => (
            ...
            >
              <div className={cn(
                ...
                msg.role === "agent" ? "bg-[hsl(var(--primary))/0.15]" : "bg-[hsl(var(--bg-elevated))]"
              )}>
                {msg.role === "agent" ? (
              ...
              <div className={cn(
                ...
                msg.role === "agent"
                  ? "bg-[hsl(var(--primary))/0.1] rounded-tl-sm text-[hsl(var(--text))]"
                  : "bg-[hsl(var(--bg-elevated))] rounded-tr-sm text-[hsl(var(--text))]"
              )}>
